...
/** One file this store touched. */
export interface WrittenFile {
  /** Workspace-relative path as the caller supplied it. */
  path: string
  /** Whether the write created the file or replaced it. */
  operation: 'create' | 'update'
}

/** Workspace-relative text storage for course files. */
export interface NoteStore {
  /**
   * Read a workspace-relative text file.
   *
   * @param path Workspace-relative path.
   * @param exec The running tool call (cwd, sandbox policy, signal, agent).
   * @returns The file text, or `undefined` when it does not exist.
   */
  read(path: string, exec: ToolRunContext): Promise<string | undefined>

  /**
   * Create or replace a workspace-relative text file.
   *
   * @param path Workspace-relative path.
   * @param content Full file content.
   * @param exec The running tool call.
   * @returns The path plus the provider's create/update verdict.
   */
  write(path: string, content: string, exec: ToolRunContext): Promise<WrittenFile>
}
 ...
 */
export function createNoteStore(ctx: Context): NoteStore {
  ...
  const targetOf = async (
    path: string,
    exec: ToolRunContext,
  ): Promise<{ target: FsTarget; sandbox: SandboxExecutionPolicy | undefined }> => {
  ...
  return {
    async read(path, exec) {
      ...
      if (info.type !== 'file') throw new Error(`"${target.displayPath}" is a ${info.type}, not a file`)
...
export async function appendFile(
  ...
  header: string,
  exec: ToolRunContext,
): Promise<{ file: WrittenFile; previous: string | undefined }> {

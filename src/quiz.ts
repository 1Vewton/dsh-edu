...
import { appendFile } from './store.ts'
import { courseLayout, renderQuizSection, stamp } from './paths.ts'
import type { QuizLogEntry } from './paths.ts'

/** One question the model asked the learner. */
export interface QuizQuestion {
  id: string
  question: string
  /** Choice labels; absent for an open question answered in free text. */
  options?: readonly string[]
  /** Acceptable labels: for a single-select question, any of them is correct. */
  answer: readonly string[]
  /** Shown once the learner has answered. */
  explanation?: string
  /** Whether several labels must be selected together. */
  multiSelect?: boolean
}

/** One answer as the interaction channel reported it. */
...
export type QuizVerdict = 'correct' | 'partial' | 'incorrect' | 'skipped' | 'needs_review'
...
export interface QuizOutcome {
  ...
  question: string
  verdict: QuizVerdict
  /** Acceptable labels, in the order the model declared them. */
  expected: readonly string[]
  /** Labels the learner selected. */
  selected: readonly string[]
  /** Free-text answer, empty when the learner chose labels. */
  custom: string
  /** The learner's answer, formatted for reading. */
  responded: string
  /** Empty when the model supplied none. */
  explanation: string
}

/** The whole grading result of one quiz. */
export interface QuizResult {
  outcomes: QuizOutcome[]
  /** Questions that could be auto-graded (they declared options). */
  gradable: number
  correct: number
  partial: number
  incorrect: number
  skipped: number
  /** Open questions whose free-text answer the model must judge itself. */
  needsReview: number
  /** `correct + 0.5 × partial` over `gradable`; `undefined` when nothing was auto-gradable. */
  score: number | undefined
  /** `score` against the pass ratio; `undefined` when there is no score. */
  passed: boolean | undefined
  /** Questions that were not answered correctly, for re-teaching. */
  weakPoints: string[]
}
...
export function gradeQuiz(
  ...
  const outcomes: QuizOutcome[] = questions.map((question) => {
  ...
  const count = (verdict: QuizVerdict): number => outcomes.filter(outcome => outcome.verdict === verdict).length
...
export function renderQuizResult(topic: string, result: QuizResult, logPath: string): string {
  const lines: string[] = []
  ...
  for (const outcome of result.outcomes) {
    ...
    if (outcome.verdict !== 'correct' && outcome.expected.length > 0) parts.push(`  expected: ${outcome.expected.join(' / ')}`)
    if (outcome.explanation !== '') parts.push(`  explanation: ${outcome.explanation}`)
...
function logEntry(outcome: QuizOutcome): QuizLogEntry {
  return {
...
/** Dependencies the quiz tool needs from the plugin. */
export interface QuizToolDeps {
  /** Hard ceiling on one quiz's question count. */
  maxQuestions: number
  /** Pass ratio used when the model does not pass one. */
  passRatio: number
  /**
   * Course-file store, read lazily: the filesystem child may activate after
   * this tool is registered, and a composition without `fs` still quizzes —
   * it just cannot log the result.
   *
   * @returns The store, or `undefined` when no filesystem is composed.
   */
  store(): NoteStore | undefined
  /** Notes root, for path reporting. */
  notesDir: string
  /** The session's active course, or `undefined` when none was opened. */
 ...
 */
export function registerQuizTool(ctx: Context, deps: QuizToolDeps): void {
  ctx.tools.register(defineTool({
      ...
      render: (_args, value) => {
        ...
        const result: QuizResult = {
          outcomes: value.results.map(row => ({
            ...
            verdict: row.verdict as QuizVerdict,
    ...
    execute: async (args, exec) => {
      ...
      if (interaction === undefined) return noChannelResult(questions, course, args.topic)

      const request: AskUserQuestionRequest = {
        questions: questions.map((question): AskUserQuestionItem => ({
          id: question.id,
          header: args.topic,
          question: question.question,
      ...
      try {
        ...
        if (CANCELLED_CODES.has(code)) return dismissedResult(course, args.topic)
        // No human can answer here (no UI provider, or a delegated caller):
        // hand the questions back instead of failing the call.
        if (NO_CHANNEL_CODES.has(code)) return noChannelResult(questions, course, args.topic)
        throw error
      }
      ...
      if (store !== undefined) {
        const { file } = await appendFile(
          store,
          courseLayout(deps.notesDir, course).quizzes,
          `${section}\n`,
...
function noChannelText(topic: string): string {
 ...
 */
function dismissedResult(course: string, topic: string) {
  return {
...
function normalizeQuestions(
  ...
  if (raw.length === 0) throw new Error('edu_quiz needs at least one question')
  ...
  return raw.map((question, index) => {
    ...
    if (text === '') throw new Error(`${id}: every quiz question needs non-empty text`)
    ...
    if (answer.length === 0) throw new Error(`${id}: every quiz question needs at least one acceptable answer`)
...
function resolvePassRatio(requested: number | undefined, fallback: number): number {

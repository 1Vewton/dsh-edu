...
/**
 * Placeholders substituted into {@link DEFAULT_EDU_POLICY} at plugin load.
 * Kept deliberately tiny: only values a deployment can actually change.
 */
export interface PolicyTokens {
  /** Workspace-relative notes root the notes tools write into. */
  notesDir: string
  /** Hard ceiling the quiz tool enforces on one quiz. */
 ...
 */
export function renderDefaultPolicy(tokens: PolicyTokens): string {

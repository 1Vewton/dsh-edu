...
import { registerCourseTools } from './course.ts'
import { renderDefaultPolicy } from './policy.ts'
import { registerQuizTool } from './quiz.ts'
import { createNoteStore } from './store.ts'
import type { NoteStore } from './store.ts'

declare module '@deepseek-ai/dsh-session/types' {
  interface SessionEventMap {
    /**
     * Whether education mode is in force from this point on, and which course
     * it is teaching: log-only, non-surface, whole-value replace. The last
     * `edu/mode` wins; a log with none folds to inactive.
     */
    'edu/mode': { active: boolean; course?: string }
  }
}
...
/** Education mode state as recorded in the session log. */
export interface EduState {
  /** Whether the mode is in force. */
  active: boolean
  /** The course being taught, when one was opened. */
  course?: string
}

/** Deployment-owned education-mode configuration. */
export interface EduModeConfig {
  /** Guidance section rendered while the mode is active. Defaults to the built-in teaching protocol. */
  policy?: string
  /** Workspace-relative directory course files are written under. */
  notesDir?: string
  /** Hard ceiling on how many questions one `edu_quiz` call may ask. */
  maxQuizQuestions?: number
  /** Default pass ratio for `edu_quiz`. */
  passRatio?: number
}

/** Validated configuration with defaults applied. */
export interface ResolvedEduConfig {
  policy?: string
  notesDir: string
  maxQuizQuestions: number
  passRatio: number
...
const EMPTY_CONFIG: EduModeConfig = {}
 ...
 */
export function resolveConfig(config: EduModeConfig = EMPTY_CONFIG): ResolvedEduConfig {
  ...
  if (notesDir === '') throw new Error('EduModeConfig needs a non-empty `notesDir`')
 ...
 */
export function foldEduMode(events: readonly SessionEvent[], end = events.length): EduState {
  let state: EduState = { active: false }
  let index = 0
  for (const event of events) {
    if (index >= end) break
    index++
    if (event.type !== 'edu/mode') continue
...
function sameState(left: EduState, right: EduState): boolean {
...
function stateData(state: EduState): { active: boolean; course?: string } {
...
function modeAtLastHeader(events: readonly SessionEvent[]): EduState | undefined {
  let lastHeader = -1
  let index = 0
  for (const event of events) {
...
export class EduMode extends Service {
  ...
  /** Validated configuration with defaults applied. */
  readonly config: ResolvedEduConfig
   ...
   */
  private readonly pendingIntents = new WeakMap<Session, EduState>()

  constructor(ctx: Context, config: EduModeConfig = EMPTY_CONFIG) {
    super(ctx, 'eduMode')
    ...
    ctx.inject(['commands'], (commandCtx) => {
      commandCtx.commands.register({
        ...
        handler: ({ agent, rawInput, attachments }) => {
          ...
          const next: EduState = message === '' ? { active: true } : { active: true, course: message }
   ...
   */
  private effective(session: Session): EduState {
   ...
   */
  get(agent: Agent): EduState & { pending?: boolean } {
    const pending = this.pendingIntents.get(agent.session)
   ...
   */
  courseOf(agent: Agent | undefined): string | undefined {
   ...
   */
  setCourse(agent: Agent | undefined, course: string): 'committed' | 'queued' | 'cancelled' | 'noop' | 'detached' {
    if (agent === undefined) return 'detached'
   ...
   */
  set(agent: Agent, next: EduState): 'committed' | 'queued' | 'cancelled' | 'noop' {
    const session = agent.session
    const targeted = this.effective(session)
    if (sameState(targeted, next)) return 'noop'
  ...
  private narration(session: Session, target: EduState): UserMessage | undefined {
    const told = modeAtLastHeader(session.events)
    if (told === undefined || sameState(told, target)) return undefined

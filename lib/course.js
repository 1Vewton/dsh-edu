...
import { courseLayout, parseModules, renderNoteSection, renderSyllabus, stamp, toggleModule, upsertSection, } from "./paths.js";
 ...
 */
export function registerCourseTools(ctx, deps) {
    ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (args.action === 'open') {
                ...
                const declared = args.modules === undefined || args.modules.length === 0
                ...
                for (const module of declared) {
                    if (module.id === '')
                        throw new Error('edu_course: every module needs a non-empty id');
                    if (module.title === '')
                        throw new Error(`edu_course: module "${module.id}" needs a non-empty title`);
                }
                const ids = new Set(declared.map(module => module.id));
                if (ids.size !== declared.length)
                    throw new Error('edu_course: module ids must be unique');
            ...
            if (moduleId === '')
                throw new Error('edu_course action "complete" needs the module identifier in `module`');
    ...
    ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (lesson === '')
                throw new Error('edu_notes needs a non-empty lesson title');
            if (args.markdown.trim() === '')
                throw new Error('edu_notes needs the lesson body in `markdown`');
            const layout = courseLayout(deps.notesDir, course);
            ...
            const file = await deps.store.write(layout.notes, upsert.text, exec);
            return {
                course,
                lesson,
                path: file.path,
